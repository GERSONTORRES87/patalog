...
def inicio(request):
    ...
    proximas = RegistroVacuna.objects.filter(proxima_dosis__gte=date.today()).order_by(
        "proxima_dosis"
    )[:5]
    mascotas_recientes = Mascota.objects.order_by("-creado")[:4]
    return render(
        request,
        "mascotas/inicio.html",
        {
...
def agregar_vacuna(request, pk):
    ...
    return render(
        request, "mascotas/vacuna_form.html", {"form": form, "mascota": mascota}
    )
...
def eliminar_vacuna(request, pk):
    ...
    return render(
        request, "mascotas/vacuna_confirm_delete.html", {"registro": registro}
    )
